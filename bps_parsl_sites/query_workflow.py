...
def query_workflow(run_id, db_file="./runinfo/monitoring.db"):
    """
    ...
    with sqlite3.connect(db_file) as conn:
        df = pd.read_sql("select * from workflow where " f'run_id="{run_id}"', conn)
        if df.empty:
            raise FileNotFoundError(f"run_id {run_id}" "not in {db_file}")
    ...
    for _, row in df0.iterrows():
        if (
            row["task_stderr"] in task_stderrs
            and row["task_status_name"] != "exec_done"
        ):
            continue
